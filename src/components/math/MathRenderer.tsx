  ...
  const processedContent = React.useMemo(() => {
    ...
    return parts.map((part, index) => {
      ...
      if (/^\$.*\$$/.test(trimmed)) {
        ...
        return <InlineMath key={index} math={equation} errorColor="#cc0000" />;
      }

      // Regular text
      return <p key={index} className="text-gray-700 whitespace-pre-wrap leading-relaxed">{trimmed}</p>;
    });
  }, [content]);

  return (
    <div className={`math-renderer space-y-4 ${className}`}>
      {processedContent}
    </div>
  );
}