...
export function MathContentParser({ content, className = '' }: MathContentParserProps) {
      ...
      if (trimmed.startsWith('```math') || /^\\\[[\s\S]*\\\]$/.test(trimmed)) {
        ...
        return (
          <div key={index} className="my-4 px-4 py-2 bg-gray-50 rounded-lg overflow-x-auto">
            <BlockMath math={cleanMathExpression(equation)} errorColor="#cc0000" />
          </div>
        );
      }
      ...
      if (/^\\\(.*\\\)$/.test(trimmed)) {
        ...
        return (
          <span key={index} className="inline-math px-1">
            <InlineMath math={cleanMathExpression(equation)} errorColor="#cc0000" />
          </span>
        );
      }
